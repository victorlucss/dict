...
```bash
cd src-tauri && cargo tauri build
# Output: src-tauri/target/release/bundle/dmg/MegaBrain_X.Y.Z_aarch64.dmg
```
...
Global Hotkey (tauri-plugin-global-shortcut; bare modifiers via native NSEvent monitor on macOS)
    → start_recording() / stop_recording()
        → Whisper (audio.rs capture → 16kHz f32 samples → whisper.rs in-process whisper-rs, warm model)
            → LLM Processor (llm.rs: MegaBrain Cloud / OpenAI / Anthropic / OpenRouter / Ollama / LM Studio)
...
MegaBrain Cloud (llm.rs → Convex backend: managed cleanup behind a per-user feature flag)
```
...
- `fallback/` — Copy-fallback popup shown when MegaBrain can't paste into the focused app
...
| `llm.rs` | LLM post-processing (6 providers: MegaBrain Cloud, OpenAI, Anthropic, OpenRouter, Ollama, LM Studio; correction levels 1-5; tone) |
